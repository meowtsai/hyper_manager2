          ...
          <small>
            {
              vipServiceOptions.filter(
                (item) => item.type == row.service_type
              )[0].label
            }
          </small>
        );
    ...
    },
    {
      ...
      formatter: (cellContent, row) => {
        return (
          <small>
            {
              vipServiceOptionsByGameId[gameId][row.service_type][
                row.request_code
              ]
            }
          </small>
        ...
        <CSVLink
          data={arrangedData
            .map((record) => ({
              ...
              code:
                vipServiceOptionsByGameId[gameId][record.service_type][
                  record.request_code
                ],
            }))
